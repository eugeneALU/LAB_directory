...
db_file = opts.file
server_address = ("", opts.port)    #why servr name here is ""
...
class Request(threading.Thread):
    ...
    def process_request(self, request):
        ...
        """
        ask = json.loads(request)
        try:
            if (ask["method"] == "read"):
                data, result = self.db_server.read()
                print(data,result)
                if (result == "success"):
                    return json.dumps({"result" : data})
                else:
                    return json.dumps({"error": {"name": type(data),"args": data}})
            elif (ask["method"] == "write"):
                result = self.db_server.write(ask["args"])
                if (result == "success"):
                    return json.dumps({"result" : result})
                else:
                    return json.dumps({"error": {"name": type(result),"args": result}})

        except Exception as e:
            
            return json.dumps({"error": {"name": type(e),"args": e}})
