...
class DatabaseProxy(object):
    ...
    def read(self):
        try:
            ...
            read = {'method':'read', 'args': None}
            data = json.dumps(read)  #adding following will crash why? {indent = 4, separators = (',',':')}
            worker.write(data + '\n')
            worker.flush()
            ...
            return "error name: " + str(type(e).__name__) + " error args: "+ str(e.args)
    ...
    def write(self, fortune):
        try:
            ...
            write = {'method':'write', 'args': fortune}
            data = json.dumps(write)
            worker.write(data + '\n')
            worker.flush()
            receive = json.loads(worker.readline())
            if ("result" in receive):
