...
#if LOADBALANCE == 1
/* use for cyclic scheduling (almost same with original version) */ 
    // Iterate hrough lines
	for (i = args->begin_h; i < args->end_h; i++)
	{
        // Iterate through pixels in a line 
		/* change the {iteration step} to achieve cyclic scheduling */
        for (j = args->begin_w; j < args->end_w; j=j+NB_THREADS)
        {
...
/* naive parallel implementation */
	...
	// every thread own whole line (row-wise segmentation)
	parameters->begin_w = 0;
	parameters->end_w = parameters->width;

	if(args->id == NB_THREADS - 1)
	{
		parameters->begin_h = chunk_height * args->id;
		parameters->end_h = parameters->height;				//handle the remaining
	}
	else
	{
		parameters->begin_h = chunk_height * args->id;
		parameters->end_h = chunk_height * (args->id + 1);
	}
	// Go
...
/* Cyclic load balance */
	...
	// start from different point, process later in compute_chunk
	parameters->begin_w = args->id;
	parameters->end_w = parameters->width;
...
/* Dynamic Scheduling*/
    ...
    int COUNT_MAX = 50;
    int local_count;		//create a local version of count, in case "race condition" happen
    int chunk_height = parameters->height/COUNT_MAX;
    ...
    while(1) {
	    ...
	    parameters->begin_h = chunk_height * (local_count-1);
	    parameters->end_h = chunk_height * local_count;

        if (local_count-1==COUNT_MAX) {
            parameters->end_h = parameters->height;
        }
