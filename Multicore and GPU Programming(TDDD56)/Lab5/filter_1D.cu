                       ...
                       const int kernelsize, const int intervalx, const int intervaly)
{
	...
	int divby = (2*kernelsize+1); // Works for box filters only!
    ...
    //idx = localy*length+x+offset;
		...
		sumx += local_mem[idx*3+0];
		sumy += local_mem[idx*3+1];
		sumz += local_mem[idx*3+2];
	}

    out[((yy)*intervaly+(xx)*intervalx)*3+0] =  sumx/divby;
	out[((yy)*intervaly+(xx)*intervalx)*3+1] =  sumy/divby;
	out[((yy)*intervaly+(xx)*intervalx)*3+2] =  sumz/divby;
}
