...
__global__ void filter(unsigned char *image, unsigned char *out, const unsigned int imagesizex, const unsigned int imagesizey, const int kernelsizex, const int kernelsizey)
{
  ...
  int realx = localx - kernelsizex + startx - 1;  //real image idx that local idx corresponding to
  int realy = localy - kernelsizey + starty - 1;  //??
  ...
  __syncthreads();
	...
	for(dy=-kernelsizey;dy<=kernelsizey;dy++)
	{
		...
		{
      x = min(max(localx+kernelsizex+dx, 0), BLOCKSIZE-1);
      y = min(max(localy+kernelsizey+dy, 0), BLOCKSIZE-1);
