...
// Size of data!
#define kDataLength 131072 //512*256 = 131072
...
void runKernel(cl_kernel kernel, cl_kernel kernel_512,int threads, cl_mem data, unsigned int length)
{
  ...
  int max_step = (log2(kDataLength/512));
  if (length < 512) {max_step = 0;}
  printf("MAXSTEP: %d\n",max_step);
  //adjust the lenth each block should handle
  int start = 2;
  int j,k;
  ...
  // set the args values
  ciErrNum  = clSetKernelArg(kernel_512, 0, sizeof(cl_mem),  (void *) &data);
  ciErrNum  = clSetKernelArg(kernel_512, 1, sizeof(cl_uint),  (void *) &localWorkSize);
  ciErrNum  = clEnqueueNDRangeKernel(commandQueue, kernel_512, 1, NULL, &globalWorkSize, &localWorkSize, 0, NULL, &event);

  if (length > 512){
      ciErrNum  = clSetKernelArg(kernel, 0, sizeof(cl_mem),  (void *) &data);
      for (k=1024;k<=length;k=2*k)
      {
        for (j=k>>1;j>0;j=j>>1)
        {
...
static cl_kernel gpgpuSort, gpgpuSort_512;
...
int bitonic_gpu(unsigned int *data, unsigned int length)
{
	...
	// ********** RUN THE KERNEL ************
	runKernel(gpgpuSort, gpgpuSort_512,length, io_data, length);
...
int main( int argc, char** argv)
{
  ...
  gpgpuSort_512 = compileKernel("bitonic.cl", "bitonic_512");
...
/*  printf("CPU:\n");
  ...
  printf("GPU:\n");
  for (int i=0;i<512;i++)
    printf("%5u %5u %5u %5u\n", data_gpu[i],data_gpu[i+512],data_gpu[i+1024],data_gpu[i+1536]);
  printf("\n");*/
  ...
  if (gpgpuSort_512) clReleaseKernel(gpgpuSort_512);
  return 0;
}
